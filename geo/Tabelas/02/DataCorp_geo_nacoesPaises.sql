  ...
  EXEC ('
    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN TRY
          SET @erroNumero = 1
          SET @mensagem = ''Execução: [geo].[paisExiste] ( @mestreNome ) não existe!''
          IF  [geo].[paisExiste] ( @mestreNome ) = 1
          BEGIN
            SET @erroNumero = 2
            SET @mensagem = ''Execução: [geo].[paisId] ( @mestreNome ) não encontrado!''
            ...
            BEGIN
              ...
              BEGIN
                ...
                BEGIN
                  SET @erroNumero = 5
                  SET @mensagem = ''Execução: [DataCorp].[geo].[nacaoPaisExiste] ( @mestreNome, @detalheNome )''
                  IF  [DataCorp].[geo].[nacaoPaisExiste] ( @mestreNome, @detalheNome ) = 0
                  BEGIN
                    SET @erroNumero = 6
                    SET @mensagem = ''Execução: INSERT INTO  [geo].[nacoesPaises] VALUES ( @paiId , @nacId )''
                    INSERT
        ...
        BEGIN CATCH
          ...
          IF (XACT_STATE()) = -1
          BEGIN
            PRINT ''[geo].[nacaoPaisInclui] não pode ser concluída. Rolling back transaction.''
